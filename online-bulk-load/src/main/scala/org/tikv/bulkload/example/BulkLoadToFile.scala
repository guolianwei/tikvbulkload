...
object BulkLoadToFile {
  ...
  def main(args: Array[String]): Unit = {
    ...
    for (i <- 1 to 1200) {
      ...
      val rdd: Array[(Array[Byte], Array[Byte])] = {
        produceRdd(spark, sql, i)
      }
      val oFileName = "/root/bulkload/data/pdata_" + i;
      println(oFileName)
      val out = new ObjectOutputStream(new FileOutputStream(oFileName));
      out.writeObject(rdd)
      out.close()
  ...
  /*    val in = new ObjectInputStream(new FileInputStream(oFileName))
      ...
      for (elem <- saveNick) {
        println(new String(elem._1) + " -> " + new String(elem._2));
      }*/

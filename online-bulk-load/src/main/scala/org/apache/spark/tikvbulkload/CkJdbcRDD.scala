...
class JdbcPartition(idx: Int, val lower: Long, val upper: Long) extends Partition {
...
class CkJdbcRDD[T: ClassTag](
    ...
    upperBound: Long,
    numPartitions: Int,
    mapRow: (ResultSet) => T = CkJdbcRDD.resultSetToObjectArray _)
  ...
  override def compute(thePart: Partition, context: TaskContext): Iterator[T] = new NextIterator[T]
  {
    ...
    val lower: Int = part.lower.toInt
    val calendar: Calendar = Calendar.getInstance
    calendar.setTime(new Date())
    calendar.add(Calendar.MONTH, -lower)
    val time: Date = calendar.getTime
    val dft = new SimpleDateFormat("yyyyMM")
    val monCur=dft.format(time)
    val sqlCur=sql.replaceAll("\\?",monCur);
    println(s"sql $sqlCur")
    val stmt = conn.prepareStatement(sqlCur, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)
